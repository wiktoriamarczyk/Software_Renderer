...
struct simd_type_maping< float, eSimdType::SSE, 8 >
{
    ...
    FORCE_INLINE static void add(const type& A, float       B, type& R) noexcept
    {
        auto SB = _mm_set1_ps(B);
    ...
    FORCE_INLINE static void sub(const type& A, float       B, type& R) noexcept
    {
        auto SB = _mm_set1_ps(B);
    ...
    FORCE_INLINE static void mul(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_mul_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_mul_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void mul(const type& A, float       B, type& R) noexcept
    {
        auto SB = _mm_set1_ps(B);
    ...
    FORCE_INLINE static void div(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_div_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_div_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void div(const type& A, float       B, type& R) noexcept
    {
        auto SB = _mm_set1_ps(B);
        R.v[0] = _mm_div_ps(A.v[0], SB);
        R.v[1] = _mm_div_ps(A.v[1], SB);
    }
    FORCE_INLINE static void min(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_min_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_min_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void max(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_max_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_max_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_le(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmple_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_cmple_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_lt(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmplt_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_cmplt_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_ge(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmpge_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_cmpge_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_gt(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmpgt_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_cmpgt_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_eq(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmpeq_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_cmpeq_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_neq(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmpneq_ps(A.v[0], B.v[0]);
        R.v[1] = _mm_cmpneq_ps(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void rsqrt(const type& A, type& R)noexcept
    {
        R.v[0] = _mm_rsqrt_ps(A.v[0]);
    ...
    FORCE_INLINE static auto r_cast(const type& A, simd_data_type<__m128i, 2>& R)
    {
        R[0] = _mm_castps_si128(A[0]);
        R[1] = _mm_castps_si128(A[1]);
    }
    FORCE_INLINE static auto r_cast(const type& A, simd_data_type<__m128d, 2>& R)
    {
        R[0] = _mm_castps_pd(A[0]);
        R[1] = _mm_castps_pd(A[1]);
    }
    FORCE_INLINE static auto r_cast(const type& A, simd_data_type<__m128, 2>& R) { R = A; }
    FORCE_INLINE static void s_cast(const type& A, simd_data_type<__m128i, 2>& R)noexcept
    {
        R[0] = _mm_cvttps_epi32(A[0]);
        R[1] = _mm_cvttps_epi32(A[1]);
    }
    FORCE_INLINE static void s_cast(const type& A, simd_data_type<__m128d, 2>& R)noexcept
    {
        R[0] = _mm_cvtps_pd(A[0]);
        R[1] = _mm_cvtps_pd(A[1]);
    }
    FORCE_INLINE static void log(const type& A, type& R) noexcept
    {
        R[0] = emu_mm_log_ps(A[0]);
        R[1] = emu_mm_log_ps(A[1]);
    }
    FORCE_INLINE static void exp(const type& A, type& R) noexcept
    {
        R[0] = emu_mm_exp_ps(A[0]);
        R[1] = emu_mm_exp_ps(A[1]);
    }
    FORCE_INLINE static void pow(const type& A, const type& B, type& R) noexcept
    {
        R[0] = emu_mm_pow_ps(A[0], B[0]);
        R[1] = emu_mm_pow_ps(A[1], B[1]);
    }
    FORCE_INLINE static void pow(const type& A, float B, type& R) noexcept
    {
        R[0] = emu_mm_pow_ps(A[0], B);
...
struct simd_type_maping< int32_t, eSimdType::SSE, 8 >
{
    ...
    FORCE_INLINE static void add(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_add_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_add_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void add(const type& A, int32_t     B, type& R) noexcept
    {
        auto SB = _mm_set1_epi32(B);
        R.v[0] = _mm_add_epi32(A.v[0], SB);
        R.v[1] = _mm_add_epi32(A.v[1], SB);
    }
    FORCE_INLINE static void sub(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_sub_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_sub_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void sub(const type& A, int32_t     B, type& R) noexcept
    {
        auto SB = _mm_set1_epi32(B);
    ...
    FORCE_INLINE static void mul(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_mullo_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_mullo_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void mul(const type& A, int32_t     B, type& R) noexcept
    {
        auto SB = _mm_set1_epi32(B);
    ...
    FORCE_INLINE static void min(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_min_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_min_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void max(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_max_epi32(A.v[0], B.v[0]);
    ...
    FORCE_INLINE static void cmp_lt(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmplt_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_cmplt_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_ge(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_or_si128(_mm_cmpgt_epi32(A.v[0], B.v[0]), _mm_cmpeq_epi32(A.v[0], B.v[0]));
        R.v[1] = _mm_or_si128(_mm_cmpgt_epi32(A.v[1], B.v[1]), _mm_cmpeq_epi32(A.v[1], B.v[1]));
    }
    FORCE_INLINE static void cmp_gt(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmpgt_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_cmpgt_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_eq(const type& A, const type& B, type& R)noexcept
    {
        R.v[0] = _mm_cmpeq_epi32(A.v[0], B.v[0]);
        R.v[1] = _mm_cmpeq_epi32(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void cmp_neq(const type& A, const type& B, type& R)noexcept
    {
        auto ONE = _mm_set1_epi32(-1);
        R.v[0] = _mm_xor_si128(_mm_cmpeq_epi32(A.v[0], B.v[0]), ONE);
        R.v[1] = _mm_xor_si128(_mm_cmpeq_epi32(A.v[1], B.v[1]), ONE);
    }
    FORCE_INLINE static void AND(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_and_si128(A.v[0], B.v[0]);
        R.v[1] = _mm_and_si128(A.v[1], B.v[1]);
    }
    FORCE_INLINE static void OR(const type& A, const type& B, type& R) noexcept
    {
        R.v[0] = _mm_or_si128(A.v[0], B.v[0]);
    ...
    FORCE_INLINE static void set(int32_t A, type& R) noexcept
    {
        R.v[0] = _mm_set1_epi32(A);
        R.v[1] = R.v[0];
    }
    FORCE_INLINE static void shl(const type& A, int32_t B, type& R) noexcept
    {
        R.v[0] = _mm_slli_epi32(A.v[0], B);
    ...
    FORCE_INLINE static auto r_cast(const type& A, simd_data_type<__m128d, 2>& R)
    {
        R[0] = _mm_castsi128_pd(A[0]);
        R[1] = _mm_castsi128_pd(A[1]);
    }
    FORCE_INLINE static auto r_cast(const type& A, simd_data_type<__m128, 2>& R)
    {
        R[0] = _mm_castsi128_ps(A[0]);
    ...
    FORCE_INLINE static void s_cast(const type& A, simd_data_type<__m128, 2>& R)noexcept
    {
    ...
    FORCE_INLINE static void blend(const type& A, const type& B, const type& M, type& R) noexcept
    {
        R[0] = _mm_castps_si128(_mm_blendv_ps(_mm_castsi128_ps(A[0]), _mm_castsi128_ps(B[0]), _mm_castsi128_ps(M[0])));
